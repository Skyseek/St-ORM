...
class Tokenizer
{
	...
	public function parseEql($eqlQuery, $cleanWhitespace = true) 
	{
		/**
		 * Strip extra whitespace from the query
		 */
		if ($cleanWhitespace === true) {
			$eqlQuery = ltrim(preg_replace('/[\\s]{2,}/', ' ', $eqlQuery));
		}
		...
		preg_match_all('/' . $regex . '/smx', $eqlQuery, $result);


		// Loop through results. Only Add non-empty values.
		$tokens = array();
